...
const VALID_HIRE_TRANSITIONS = new Set([
...
const ApplicantDetail = ({ applicant, onUpdateStatus }) => {
  ...
  const canHire = !isTerminal && VALID_HIRE_TRANSITIONS.has(currentStatus);
  const canReject = !isTerminal && VALID_REJECT_TRANSITIONS.has(currentStatus);

  return (
    <div className="space-y-10">
      {/* Profile Summary Card */}
      <Card className="p-10 border border-slate-200 bg-white shadow-sm overflow-hidden rounded-xl">
        <div className="flex flex-col md:flex-row items-center gap-10">
          <Avatar
            name={`${applicant.first_name} ${applicant.last_name}`}
            src={applicant.avatar_url}
            size="xl"
            className="w-32 h-32 rounded-xl shadow-md border-4 border-white"
          />
          <div className="flex-grow text-center md:text-left">
            ...
            <p className="text-primary font-bold text-base uppercase tracking-normal mb-6">
          ...
          <div className="flex gap-3">
            <Button variant="outline" className="p-4 rounded-xl bg-white group">
              <MessageSquare
                size={20}
            ...
            <a href={applicant.resume_url} target="_blank" rel="noreferrer">
              <Button
                variant="primary"
                className="flex items-center gap-2 px-8 py-4 rounded-xl shadow-xl shadow-primary/30"
              >
                <Download size={20} />
                Tải hồ sơ
              </Button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 space-y-10">
          {/* Covering Letter */}
          <Card className="p-10 border border-slate-200 bg-white shadow-sm rounded-xl">
          ...
          <Card className="p-10 border border-slate-200 bg-white shadow-sm rounded-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="space-y-6">
                <div>
                  <p className="text-base font-bold text-slate-300 uppercase tracking-normal mb-2">
                    Vị trí ứng tuyển
                  </p>
              ...
              <div className="p-8 bg-slate-50 rounded-xl border border-slate-100 flex flex-col justify-center">
                <p className="text-base font-bold text-slate-400 uppercase tracking-normal mb-4 text-center">
                  Xử lý hồ sơ
                </p>
                <div className="flex gap-4">
                  {canHire && (
                    <Button
                      ...
                      className="flex-1 bg-emerald-600 text-white border-none rounded-xl py-4 flex items-center justify-center gap-2 font-bold text-sm hover:bg-emerald-700 shadow-lg shadow-emerald-500/20"
                    >
                  ...
                  {canReject && (
                    <Button
                      ...
                      className="flex-1 bg-rose-600 text-white border-none rounded-xl py-4 flex items-center justify-center gap-2 font-bold text-sm hover:bg-rose-700 shadow-lg shadow-rose-500/20"
                    >
                      <XCircle size={16} /> TỪ CHỐI
                    </Button>
                  )}
                  {isTerminal && (
                    <div className="w-full text-center text-slate-400 font-bold text-sm py-4">
                      Hồ sơ đã kết thúc
                    </div>
                  )}
                  {!isTerminal && !canHire && !canReject && (
                    <div className="w-full text-center text-slate-400 font-bold text-sm py-4">
        ...
        <div className="space-y-8">
          {/* Quick Stats */}
          <Card className="p-8 border border-slate-200 bg-white shadow-sm rounded-xl">
            <h4 className="text-sm font-bold text-slate-300 uppercase tracking-normal mb-6">
              Thông tin ứng viên
            </h4>
          ...
          <Card className="p-8 border border-slate-200 bg-white shadow-sm rounded-xl">
            ...
            <div className="space-y-6 relative before:absolute before:left-4 before:top-2 before:bottom-2 before:w-px before:bg-slate-100">
              <div className="pl-10 relative">
                ...
                <p className="text-base font-bold text-slate-900">Đã xem hồ sơ</p>
              ...
              <div className="pl-10 relative">
                ...
                <p className="text-base font-bold text-slate-400">Ứng tuyển thành công</p>
