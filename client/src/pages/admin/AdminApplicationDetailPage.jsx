...
const STATUS_OPTIONS = [
  { value: 'pending', label: 'Tính năng chờ duyệt', color: 'amber', icon: FileText },
  { value: 'reviewed', label: 'Đã xem xét', color: 'blue', icon: ShieldCheck },
  { value: 'shortlisted', label: 'Đã shortlist', color: 'indigo', icon: Star },
  { value: 'interviewing', label: 'Đang phỏng vấn', color: 'sky', icon: Calendar },
  { value: 'offered', label: 'Đã offer', color: 'emerald', icon: Send },
  { value: 'hired', label: 'Đã tuyển', color: 'green', icon: CheckCircle },
  { value: 'rejected', label: 'Từ chối', color: 'red', icon: XCircle },
];
...
const AdminApplicationDetailPage = () => {
  ...
  const oversightModules = [
    ...
    },
    {
      ...
      description: 'Điều phối trạng thái đơn ứng tuyển và ghi chú nội bộ để tối ưu quy trình tuyển dụng.',
      icon: Activity
    },
    {
  ...
  const candidate = useMemo(() => {
    ...
    return {
      name: `${application.first_name || ''} ${application.last_name || ''}`.trim() || 'Ứng viên chưa xác định',
  ...
  const handleStatusUpdate = async (newStatus, customOfferDetails = null) => {
    if (newStatus === 'offered' && !customOfferDetails) {
    ...
    try {
      await adminService.updateApplicationStatus(id, newStatus, note, customOfferDetails);
      showNotification(`Đã chuyển trạng thái sang ${getStatusLabel(newStatus)}`, 'success');
      setNote('');
      ...
      if (applicationResponse.data?.success) {
        ...
        setStatus(applicationResponse.data.data.status || newStatus);
  ...
  if (!application) return <div className="p-10 text-center font-bold">Không tìm thấy hồ sơ ứng tuyển</div>;

  return (
    <div className="mx-auto max-w-7xl animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-8 pb-20">
      {/* Overview Modules */}
      <div className="grid gap-4 xl:grid-cols-3">
        {oversightModules.map((module) => {
          const Icon = module.icon;
          return (
            <div key={module.title} className="group relative overflow-hidden rounded-[32px] border border-slate-200/60 bg-white p-8 shadow-sm transition-all hover:shadow-md">
              <div className="absolute -right-4 -top-4 h-24 w-24 rounded-full bg-slate-50 transition-transform group-hover:scale-110" />
              <div className="relative">
                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-slate-900 text-white shadow-lg shadow-slate-200">
                  <Icon size={22} />
                </div>
                <h3 className="mt-6 text-xl font-bold text-slate-900">{module.title}</h3>
                <p className="mt-3 text-base leading-relaxed text-slate-500 font-medium">{module.description}</p>
              </div>
            </div>
          );
        })}
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 py-2">
        <button
          ...
          className="flex items-center gap-2 text-sm font-bold uppercase tracking-normal text-slate-400 transition-colors hover:text-slate-900"
        >
        ...
        <div className="flex items-center gap-3">
          <Button
            variant="ghost"
            className="h-12 rounded-xl px-6 font-bold text-red-500 hover:bg-red-50"
          ...
          <Button
            className="h-12 rounded-xl bg-slate-900 px-8 font-bold text-white shadow-lg shadow-slate-200 hover:bg-slate-800"
            onClick={() => handleStatusUpdate('shortlisted')}
            disabled={savingStatus}
          >
            <Star size={18} className="mr-2" /> Đưa vào Shortlist
          </Button>
        </div>
      </div>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                <div className="flex items-start gap-6">
                  <div className="relative group">
                    ...
                    <Avatar name={candidate.name} size="xl" className="relative h-28 w-28 rounded-[2rem] border-4 border-white shadow-2xl" />
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-3">
                      <h1 className="text-3xl font-bold tracking-normal text-slate-900">{candidate.name}</h1>
                      <span className="rounded-full bg-slate-100 px-3.5 py-2 text-xs font-bold uppercase tracking-normal text-slate-500 border border-slate-200/50">Ứng viên</span>
                    </div>
                    <p className="text-lg font-bold text-slate-500">{candidate.role} · <span className="text-slate-400">{application.company_name}</span></p>
                ...
                <div className="flex flex-col items-end gap-3 self-start md:self-center">
                  <p className="text-xs font-bold text-slate-400 px-2 tracking-normal uppercase">Nộp lúc: {formatDate(candidate.appliedDate)}</p>
                </div>
              </div>
            </div>
          </Card>
                ...
                {candidate.experience.length ? (
                  <div className="space-y-6">
                    {candidate.experience.map((exp, i) => (
                      <div key={i} className="group relative rounded-xl bg-slate-50 p-6 transition-colors hover:bg-slate-100/60">
                        <div className="flex justify-between items-start mb-3">
                          <div>
                            <h4 className="text-lg font-bold text-slate-900">{exp.title || exp.position}</h4>
                            <p className="font-bold text-slate-500">{exp.company}</p>
                          </div>
                          <span className="rounded-xl bg-white px-4 py-2 text-xs font-bold text-slate-400 shadow-sm ring-1 ring-slate-200/60 uppercase tracking-normal">{exp.period}</span>
                        </div>
                        <p className="text-base leading-relaxed text-slate-500 font-medium">{exp.description || exp.desc}</p>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="rounded-xl border-2 border-dashed border-slate-100 p-8 text-center text-slate-400 font-bold">Chưa cập nhật kinh nghiệm</div>
                )}
              </section>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    {candidate.education.map((edu, i) => (
                      <div key={i} className="rounded-xl bg-slate-50 p-6">
                        ...
                        <p className="mt-2 inline-block rounded-lg bg-white px-3.5 py-2 text-xs font-bold uppercase text-slate-400 shadow-sm ring-1 ring-slate-200/50 tracking-normal">{edu.period || edu.year}</p>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="rounded-xl border-2 border-dashed border-slate-100 p-8 text-center text-slate-400 font-bold">Chưa cập nhật học vấn</div>
                )}
              </section>
                ...
                <div className="flex flex-wrap gap-2.5">
                  {candidate.skills.length ? (
                    candidate.skills.map((skill, i) => (
                      <span key={i} className="rounded-xl bg-slate-900 px-5 py-2 text-sm font-bold text-white shadow-md shadow-slate-200">
                        {typeof skill === 'string' ? skill : skill.name}
                      </span>
                    ))
                  ) : (
                    <div className="rounded-xl border-2 border-dashed border-slate-100 p-8 text-center text-slate-400 font-bold w-full">Chưa liệt kê kỹ năng</div>
                  )}
                </div>
              </section>
            </div>
            ...
            <div className="space-y-5">
              ...
              <div className="p-5 rounded-xl bg-slate-900/5 border border-slate-900/10">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-lg font-bold text-slate-900 tracking-normal">{getStatusLabel(status)}</span>
                </div>
              </div>
                ...
                {hasResume ? (
                  <>
                    ...
                    >
                      ...
                      <span className="text-xs font-bold uppercase tracking-normal text-slate-500">Mở CV</span>
                    </a>
                    <a
                      href={candidate.cvUrl}
                      target="_blank"
                    ...
                    >
                      ...
                      <span className="text-xs font-bold uppercase tracking-normal text-slate-500">Tải File</span>
                    </a>
                  </>
                ) : (
                  <>
                    <div className="flex flex-col items-center justify-center gap-2 rounded-[28px] border border-dashed border-slate-200 bg-slate-50/80 p-4 text-slate-300">
                      ...
                      <span className="text-xs font-bold uppercase tracking-normal">Chưa có CV</span>
                    </div>
                    <div className="flex flex-col items-center justify-center gap-2 rounded-[28px] border border-dashed border-slate-200 bg-slate-50/80 p-4 text-slate-300">
                      <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-white shadow-sm ring-1 ring-slate-200/60">
                        <Download size={18} />
              ...
              {!hasResume && (
                <p className="text-xs font-bold text-slate-400">
                  Hồ sơ này chưa có tệp CV đính kèm, nên admin đang duyệt theo dữ liệu profile hiện có.
                </p>
              )}
            </div>
          </Card>
            ...
            <div className="max-h-[400px] overflow-y-auto p-8 pt-6 scrollbar-hide">
              <div className="relative ml-2 space-y-8 border-l-2 border-slate-100">
                <div className="relative pl-8">
                  ...
                  <p className="mt-0.5 text-xs font-bold text-slate-400">{formatDate(application.applied_at || application.created_at)}</p>
                </div>
                ...
                {history.map((item, idx) => (
                  <div key={item.id || idx} className="relative pl-8">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {item.changed_by_name && <span className="text-xs font-bold text-slate-400 tracking-normal uppercase px-1.5 py-0.5 rounded-md bg-slate-50 border border-slate-100">Bởi {item.changed_by_name}</span>}
                    </div>
                    {item.notes && <p className="mt-2 rounded-xl bg-slate-50 p-4 text-xs font-bold text-slate-500 italic border border-slate-100/60 leading-relaxed">"{item.notes}"</p>}
                    <p className="mt-1.5 text-xs font-bold text-slate-300 uppercase tracking-normal">{formatDate(item.created_at)} · {formatTimeAgo(item.created_at)}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      {showOfferModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative w-full max-w-lg animate-in zoom-in-95 fade-in duration-300">
            <Card className="border-none shadow-2xl p-0 overflow-hidden rounded-[32px]">
              <div className="bg-slate-900 p-8 text-white">
                <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-white/10 mb-6">
                  <Send size={28} className="text-emerald-400" />
                </div>
                <h3 className="text-2xl font-bold tracking-normal">Cấu hình Thư mời (Offer)</h3>
                <p className="mt-2 text-slate-400 font-medium">Nhập thông tin chi tiết trước khi gửi thông báo tới ứng viên.</p>
              </div>
              
              <div className="p-8 space-y-6 bg-white">
                <div className="space-y-4">
                  <div className="space-y-2">
                    <label className="text-sm font-bold uppercase tracking-normal text-slate-400 ml-1">Mức lương đề nghị</label>
                    <div className="relative">
                      <input
                        type="text"
                        value={offerDetails.salary}
                        onChange={(e) => setOfferDetails({...offerDetails, salary: e.target.value})}
                        placeholder="VD: 25,000,000 VND / tháng"
                        className="w-full h-14 pl-12 pr-5 rounded-xl bg-slate-50 border-2 border-slate-100 font-bold text-slate-900 focus:border-slate-900 outline-none transition-all"
                      />
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-bold uppercase tracking-normal text-slate-400 ml-1">Ngày bắt đầu dự kiến</label>
                    <div className="relative">
                      <input
                        type="date"
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-bold uppercase tracking-normal text-slate-400 ml-1">Ghi chú kèm theo</label>
                    <textarea
                      value={offerDetails.notes}
                      onChange={(e) => setOfferDetails({...offerDetails, notes: e.target.value})}
                      placeholder="Các điều khoản bổ sung hoặc lời nhắn..."
                      className="w-full h-28 p-5 rounded-xl bg-slate-50 border-2 border-slate-100 font-bold text-slate-900 focus:border-slate-900 outline-none transition-all resize-none"
                    />
                  </div>
                </div>
                ...
                <div className="flex gap-3 pt-4">
                  <Button
                    variant="ghost"
                    className="flex-1 h-14 rounded-xl font-bold text-slate-500 hover:bg-slate-50"
                    onClick={() => setShowOfferModal(false)}
                  >
                    Hủy bỏ
                  </Button>
                  <Button
                    className="flex-1 h-14 rounded-xl bg-emerald-500 text-white font-bold hover:bg-emerald-600 shadow-lg shadow-emerald-100"
                    onClick={() => handleStatusUpdate('offered', offerDetails)}
                    disabled={savingStatus}
                  >
                    {savingStatus ? 'Đang gửi...' : 'Xác nhận Offer'}
                  </Button>
                </div>
              </div>
            </Card>
