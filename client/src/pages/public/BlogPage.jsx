      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {popularTags.map((tag) => (
            <span
              key={tag}
              className="cursor-default rounded-md border border-border/50 bg-slate-50 px-2 py-1 text-xs font-medium text-slate-600"
            >
              #{tag}
            </span>
          ))}
    ...
    <div className="min-h-screen bg-emerald-50/30">
      ...
      <section className="page-hero-bg page-hero-grain relative overflow-hidden">
        ...
        <div className="relative border-b border-border/50 bg-white/60 backdrop-blur-sm">
          <div className="container mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-2 px-4 py-2.5 text-[11px] font-medium uppercase tracking-[0.16em] text-muted-foreground sm:px-6 lg:px-8">
            <span>{editionDate}</span>
            <span className="hidden sm:inline">Hà Nội · TP.HCM · Đà Nẵng</span>
            <span className="font-semibold text-primary">Bản tin tuyển dụng &amp; công nghệ</span>
          </div>
        </div>
        ...
        <div className="container relative z-10 mx-auto max-w-6xl px-4 pt-12 pb-14 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-2xl text-center">
            ...
            <h1 className="text-3xl font-bold tracking-tight text-foreground sm:text-4xl md:text-[2.75rem] md:leading-[1.1]">
        ...
        <div className="container relative mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-12">
              ...
              <div className="sticky top-24 space-y-8">
                ...
                <div className="rounded-2xl border border-border/60 bg-white p-5 shadow-sm">
                  ...
                  <ol className="space-y-4">
                    {popularLoading ? (
                      <li className="text-sm text-muted-foreground">Đang tải…</li>
                    ) : popularPosts.length === 0 ? (
                      <li className="text-sm text-muted-foreground">Chưa có dữ liệu.</li>
                    ) : (
                      popularPosts.map((p, i) => (
                        <li key={p.slug || p.id} className="flex gap-3 text-sm">
            ...
            <Sheet open={mobileFilterOpen} onOpenChange={setMobileFilterOpen}>
              <SheetContent side="left" className="flex w-full max-w-sm flex-col p-0">
                <SheetHeader className="shrink-0 border-b px-6 py-4">
                  <SheetTitle>Chuyên mục</SheetTitle>
                </SheetHeader>
            ...
            <div className="flex flex-col gap-8 lg:col-span-9">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between lg:gap-6">
                <div className="flex min-w-0 flex-1 flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
                  <p className="shrink-0 text-sm text-muted-foreground">
                    ...
                    {listSource === 'fallback' && (
                      <span className="ml-2 text-xs text-amber-700">· ngoại tuyến (mẫu)</span>
                    )}
                  </p>
                  <div className="relative min-w-0 max-w-md flex-1">
                    ...
                    <Input
                      ...
                      className="h-10 border-border/60 bg-white pl-10 text-sm"
              ...
              {listSource === 'loading' ? (
                <div className="rounded-2xl border border-border/60 bg-white p-16 text-center font-serif text-muted-foreground">
                  Đang tải số báo…
                </div>
              ) : posts.length === 0 ? (
                ...
                <>
                    ...
                    <article className="overflow-hidden rounded-2xl border-2 border-foreground/10 bg-white shadow-[0_20px_50px_-24px_rgba(0,0,0,0.25)]">
                      <div className="grid gap-0 lg:grid-cols-12">
                        <div className="relative lg:col-span-7">
                          ...
                          <div className="absolute left-4 top-4 rounded bg-foreground px-3 py-1.5 font-sans text-[10px] font-bold uppercase tracking-widest text-background">
                            {featured.category}
                          </div>
                        </div>
                        <div className="flex flex-col justify-center border-t border-border/60 p-6 sm:p-8 lg:col-span-5 lg:border-l lg:border-t-0">
                          ...
                          <h2 className="font-serif text-2xl font-bold leading-[1.15] tracking-tight text-foreground sm:text-3xl">
                            <Link
                          ...
                          <p className="mt-4 line-clamp-5 font-article text-base leading-relaxed text-foreground/80">
                            {featured.excerpt}
                          </p>
                          <div className="mt-6 flex flex-wrap items-center gap-4 border-t border-border/50 pt-4 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
                          ...
                          >
                            <div className="relative aspect-[5/3] overflow-hidden bg-muted/40">
                              <img
                                ...
                                decoding="async"
                              />
                              <span className="absolute left-2 top-2 rounded bg-primary/90 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-primary-foreground">
                                {post.category}
                              </span>
                            </div>
                            <div className="flex flex-1 flex-col p-3">
                              ...
                              <p className="mt-auto pt-2 text-[10px] font-medium text-muted-foreground">
                                {post.date}
                              </p>
                            </div>
                          </Link>
                  ...
                  {moreGrid.length > 0 ? (
                    <div>
                      <h3 className="mb-4 flex items-center gap-2 border-b-2 border-foreground/80 pb-2 font-serif text-xl font-bold text-foreground">
                        Thêm bài viết
                      </h3>
                          ...
                          >
                            <div className="relative h-40 overflow-hidden border-b border-border/40 sm:h-44">
                              <img
                                ...
                                decoding="async"
                              />
                              <div className="absolute left-3 top-3 rounded bg-white/95 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-slate-800 shadow-sm">
                                {post.category}
                              </div>
                            </div>
                            <div className="flex flex-1 flex-col p-5">
                              ...
                              <p className="line-clamp-3 flex-1 font-article text-sm leading-relaxed text-foreground/75">
                                {post.excerpt}
                              </p>
              ...
              >
                ...
                <div className="relative z-10 px-6 py-10 sm:px-10 sm:py-12">
                  <div className="mx-auto max-w-2xl text-center">
                    <p className="mb-2 text-[10px] font-bold uppercase tracking-[0.22em] text-emerald-400/95">
                      Bản tin HireAI
                    </p>
                    <h3
                      id="blog-newsletter-title"
                      className="font-serif text-2xl font-bold leading-tight tracking-tight text-white sm:text-[1.75rem] text-balance"
                    >
                    ...
                    <div className="flex flex-col gap-2 sm:flex-row sm:gap-0 sm:overflow-hidden sm:rounded-xl sm:ring-1 sm:ring-white/20 sm:shadow-lg">
                      <div className="relative min-w-0 flex-1">
                        ...
                        <Input
                          ...
                          className="h-12 rounded-xl border-0 bg-white pl-11 text-sm text-slate-900 shadow-none placeholder:text-slate-400 focus-visible:ring-2 focus-visible:ring-primary/50 sm:rounded-none sm:rounded-l-xl"
                        />
                      </div>
                      <Button
                        type="submit"
                        className="h-12 shrink-0 rounded-xl px-8 font-semibold shadow-none sm:rounded-none sm:rounded-r-xl"
                      >
                        Đăng ký
                      </Button>
                    </div>
