...
class ApplicationController {
  ...
  apply = catchAsync(async (req, res) => {
    ...
    const applicationId = await ApplicationService.applyToJob(candidate.id, req.params.jobId, req.body);

    // Notification cho candidate
    await notificationService.notifyApplicationSubmitted(req.user.id, applicationId, req.params.jobId);
    ...
    if (job?.recruiter_id) {
      ...
      await notificationService.notifyNewApplicant(job.recruiter_id, applicationId, {
        candidate_name: `${candidateProfile?.first_name || ''} ${candidateProfile?.last_name || ''}`.trim() || 'Ứng viên',
        job_title: job.title,
  ...
  getMyApplications = catchAsync(async (req, res) => {
    const candidate = await CandidateRepository.findByUserId(req.user.id);
    if (!candidate) return ApiResponse.forbidden(res, 'Only candidates can view their applications');
  ...
  getMyInterviews = catchAsync(async (req, res) => {
    const candidate = await CandidateRepository.findByUserId(req.user.id);
    if (!candidate) return ApiResponse.forbidden(res, 'Only candidates can view interview schedules');
  ...
  getMyApplication = catchAsync(async (req, res) => {
    ...
    const application = await ApplicationService.getCandidateApplication(req.params.id, candidate.id);
    return ApiResponse.success(res, application);
  });

  getMyApplicationHistory = catchAsync(async (req, res) => {
    const candidate = await CandidateRepository.findByUserId(req.user.id);
    if (!candidate) return ApiResponse.forbidden(res, 'Only candidates can view application history');

    const history = await ApplicationService.getCandidateApplicationHistory(req.params.id, candidate.id);
  ...
  getJobApplications = catchAsync(async (req, res) => {
    ...
    if (!isAdmin) {
      const permissionError = this._ensureRecruiterPermission(
        ...
        'Báº¡n khÃ´ng cÃ³ quyá»n xem há»“ sÆ¡ á»©ng viÃªn'
      );
      if (permissionError) return permissionError;
    }

    const applications = await ApplicationService.getJobApplications(req.params.jobId, company?.id, isAdmin);
    return ApiResponse.success(res, applications);
  });
  ...
  getApplication = catchAsync(async (req, res) => {
    ...
    if (!isAdmin && !company) {
      return ApiResponse.forbidden(res, 'Vui lòng hoàn thiện hồ sơ công ty trước khi xem hồ sơ ứng tuyển');
    }
    ...
    if (!isAdmin) {
      const permissionError = this._ensureRecruiterPermission(
        ...
        req.user,
        'can_view_applications',
        'Báº¡n khÃ´ng cÃ³ quyá»n xem há»“ sÆ¡ á»©ng tuyá»ƒn'
      );
      if (permissionError) return permissionError;
    }

    const application = await ApplicationService.getApplication(req.params.id, company?.id ?? null, isAdmin);
    return ApiResponse.success(res, application);
  });
  ...
  updateStatus = catchAsync(async (req, res) => {
    ...
    if (!isAdmin && !company) {
      return ApiResponse.forbidden(res, 'Vui lòng hoàn thiện hồ sơ công ty trước khi cập nhật hồ sơ ứng tuyển');
    }
    ...
    if (!isAdmin) {
      const permissionError = this._ensureRecruiterPermission(
        ...
        'Báº¡n khÃ´ng cÃ³ quyá»n cáº­p nháº­t tráº¡ng thÃ¡i há»“ sÆ¡'
      );
      if (permissionError) return permissionError;
    }
  ...
  getCompanyInterviews = catchAsync(async (req, res) => {
    ...
    if (!isAdmin && !company) {
      return ApiResponse.forbidden(res, 'Vui lòng hoàn thiện hồ sơ công ty trước khi xem lịch phỏng vấn');
    }
    ...
    if (!isAdmin) {
      const permissionError = this._ensureRecruiterPermission(
        ...
        req.user,
        'can_view_applications',
        'Báº¡n khÃ´ng cÃ³ quyá»n xem lá»‹ch phá»ng váº¥n'
      );
      if (permissionError) return permissionError;
    }
  ...
  updateInterviewStatus = catchAsync(async (req, res) => {
    ...
    if (!isAdmin && !company) {
      return ApiResponse.forbidden(res, 'Vui lòng hoàn thiện hồ sơ công ty trước khi cập nhật lịch phỏng vấn');
    }
    ...
    if (!isAdmin) {
      const permissionError = this._ensureRecruiterPermission(
        ...
        req.user,
        'can_manage_applications',
        'Báº¡n khÃ´ng cÃ³ quyá»n cáº­p nháº­t lá»‹ch phá»ng váº¥n'
      );
      if (permissionError) return permissionError;
    }
  ...
  addNote = catchAsync(async (req, res) => {
    ...
    if (!isAdmin && !company) {
      return ApiResponse.forbidden(res, 'Vui lòng hoàn thiện hồ sơ công ty trước khi ghi chú hồ sơ');
    }
    ...
    if (!isAdmin) {
      const permissionError = this._ensureRecruiterPermission(
        ...
        req.user,
        'can_manage_applications',
        'Báº¡n khÃ´ng cÃ³ quyá»n ghi chÃº há»“ sÆ¡ á»©ng tuyá»ƒn'
      );
      if (permissionError) return permissionError;
    }
  ...
  getHistory = catchAsync(async (req, res) => {
    ...
    if (!isAdmin && !company) {
      return ApiResponse.forbidden(res, 'Vui lòng hoàn thiện hồ sơ công ty trước khi xem lịch sử hồ sơ');
    }
    ...
    if (!isAdmin) {
      const permissionError = this._ensureRecruiterPermission(
        ...
        req.user,
        'can_view_applications',
        'Báº¡n khÃ´ng cÃ³ quyá»n xem lá»‹ch sá»­ há»“ sÆ¡'
      );
      if (permissionError) return permissionError;
    }

    const history = await ApplicationService.getApplicationHistory(req.params.id, company?.id ?? null, isAdmin);
    return ApiResponse.success(res, history);
  });
  ...
  getInterviews = catchAsync(async (req, res) => {
    ...
    const interviews = await ApplicationService.getInterviewSchedules(req.params.id, company?.id ?? null, isAdmin);
    return ApiResponse.success(res, interviews);
  });
  ...
  getOffer = catchAsync(async (req, res) => {
    ...
    if (!isAdmin) {
      const permissionError = this._ensureRecruiterPermission(
        ...
        req.user,
        'can_view_applications',
        'Báº¡n khÃ´ng cÃ³ quyá»n xem offer'
      );
      if (permissionError) return permissionError;
    }

    const offer = await ApplicationService.getOfferDetails(req.params.id, company?.id ?? null, isAdmin);
