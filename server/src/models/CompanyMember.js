...
class CompanyMemberRepository {
  ...
  async isAdmin(companyId, userId) {
    ...
    return (
      ...
      (member?.role === COMPANY_ROLES.OWNER || member?.role === COMPANY_ROLES.ADMIN)
    );
  }
  ...
  async inviteMember(companyId, userId, role, permissions, invitedBy) {
    ...
    for (const field of PERMISSION_FIELDS) {
      perms[field] =
        permissions && permissions[field] !== undefined
          ? permissions[field]
    ...
    `,
      [
        ...
        perms.can_post_job,
        perms.can_edit_job,
        perms.can_delete_job,
        perms.can_approve_job,
        perms.can_view_applications,
        perms.can_manage_applications,
        perms.can_send_email,
        perms.can_view_salary,
        perms.can_export_data,
      ]
    );
    return this.findOne(companyId, userId);
  }
  ...
  async updateRole(companyId, userId, newRole) {
    ...
    await pool.query(`UPDATE company_members SET role = ? WHERE company_id = ? AND user_id = ?`, [
      newRole,
      companyId,
      userId,
    ]);
