...
exports.registerValidator = [
  body('email').isEmail().normalizeEmail({ gmail_remove_dots: false, gmail_remove_subaddress: false }).withMessage('Email không hợp lệ'),
  body('password')
    .isLength({ min: 8 })
  ...
  body('role').isIn(['candidate', 'recruiter', 'employer']).withMessage('Role không hợp lệ'),
  body().custom((_, { req }) => {
  ...
  body().custom((_, { req }) => {
    ...
    if (roleVal !== 'employer' && roleVal !== 'recruiter') {
      return true;
    }
