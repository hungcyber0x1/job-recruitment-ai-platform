  ...
  try {
    ...
    if (!colNames.includes('deleted_at')) {
      ...
      await connection.query(
        'ALTER TABLE employers ADD COLUMN deleted_at DATETIME DEFAULT NULL AFTER updated_at'
      );
    } else {
    ...
    if (!colNames.includes('rejection_reason')) {
      ...
      await connection.query(
        'ALTER TABLE employers ADD COLUMN rejection_reason TEXT DEFAULT NULL AFTER moderation_note'
      );
    } else {
    ...
    await connection.query(
      "ALTER TABLE users MODIFY COLUMN status ENUM('active', 'pending', 'banned', 'locked') DEFAULT 'active'"
    );
